...
class ApiRequestor
{
    ...
    private function _requestRaw($method, $url, $params, $headers)
    {
        ...
        $defaultHeaders = array(
            'X-Ads-Client-User-Agent' => json_encode($ua),
            'User-Agent' => 'Ads/v1 PhpBindings/' . Ads::VERSION,
            'Authorization' => 'Bearer ' . $myApiKey,
        );
        if (Ads::$apiVersion) {
            $defaultHeaders['Ads-Version'] = Ads::$apiVersion;
        }

        if (Ads::$accountId) {
            $defaultHeaders['Ads-Account'] = Ads::$accountId;
        }
